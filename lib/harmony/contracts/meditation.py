...
from lib.chain_apis import harmony_api
...
def get_gas_price(rpc):
    hmny_api = harmony_api.HarmonyApi(rpc)
    # Get gas price
    gas = hmny_api.get_gas_price()
    try:
        gas_data = gas.json()
        gas = gas_data['result']
        print(f"Spending {gas} gas")
    except:
        sys.exit("Can't get gas from API")
    return gas
...
def complete_meditation(hero_id, private_key, nonce, rpc_address, logger):
    w3 = Web3(Web3.HTTPProvider(rpc_address))
    account = w3.eth.account.privateKeyToAccount(private_key)
    w3.eth.default_account = account.address
